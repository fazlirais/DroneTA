...
MQTT_TOPIC = "drone/machine_learning"
Fovx = 54
Fovy = 41
cam_degree = 16
H_Plant = 0.1
START_ALT = 5 # Altitude to start from (in meters)
Drone_ALT = 1.5
...
def clear_processed_images():
    folder = r"C:\Users\ASUS\Downloads\Drone_Project\Disease_Detection_DJI\processed_images"
...
model = YOLO(r"C:\Users\ASUS\Downloads\best (2).pt")
#image_path = r"C:\Users\ASUS\Downloads\leaves.jpg"
#image = cv2.imread(image_path)

# Path to the folder containing multiple images
image_folder = r"C:\Users\ASUS\Downloads\Chest Height-20250319T070249Z-001\Chest Height 2"
terrain_image_path = r"C:\Users\ASUS\Downloads\terrain.jpg"

# Collect data for plotting
disease_data = {"all": []}# Dictionary to store coordinates per disease

def get_random_image(folder):
    images = [f for f in os.listdir(folder) if f.endswith(('.jpg', '.png'))]  # Get all image files
    if not images:
        raise Exception("No images found in the folder!")
    return os.path.join(folder, random.choice(images))  # Choose a random image
...
def upload_images_to_github():
    ...
    repo_path = r"C:\Users\ASUS\DroneTA"  # Ubah sesuai path local repo
...
prev_x_drone, prev_y_drone = 0, 0
prev_x_drone1, prev_y_drone1 = 0, 0

# Main Script
def main():
    # Reset the payload file at the start of execution
    with open("payload.txt", "w") as file:
        file.write("")  # Clears the file content
    vehicle = connect_drone()
    arm_and_takeoff(vehicle, START_ALT)
    ...
    try:
        ...
        with open(file_path, 'r') as f:
            ...
            while True:
                ...
                time.sleep(0.8)
                image_path = get_random_image(image_folder)  # Get a new random image
                ...
                for box in result.boxes:
                    ...
                    if conf >= 0.5:
                        ...
                        yavg = int((y1 + y2) / 2)
                        #x_drone, y_drone = Transformer.from_crs('epsg:4326', 'epsg:3857', always_xy=True).transform(lon_drone, lat_drone)
                        x_final, y_final = find_coordinate(xavg, yavg, x_drone, y_drone, Drone_ALT, Fovx, Fovy, yaw0, H_Plant)
